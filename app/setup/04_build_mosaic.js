...
function run (doHqVersion, mainCallback) {
	var tilesWide, tilesHeigh, mainimage, mainimagehq, mosaicimage, mosaicimage_overlayed;
	var inputfiles, inputfilesHQ;


	var now = Math.round( Date.now()/1000 );
	mosaicimage = path.join(config.mosaic.folders.output, 'mosaic_'+now+'.jpg');
	mosaicimage_overlayed = path.join(config.mosaic.folders.output, 'mosaic_'+now+'_overlayed.jpg');

	mosaicimagehq = path.join(config.mosaic.folders.output, 'mosaic_'+now+'_hq.jpg');
	mosaicimagehq_overlayed = path.join(config.mosaic.folders.output, 'mosaic_'+now+'_hq_overlayed.jpg');
	...
	async.waterfall([
		...
		function (res, $) {
			if(!doHqVersion) return $(null, null);
